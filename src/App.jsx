...
function PermissionGuard({ children, permission }) {
  const { user } = useAuth();
  if (user?.role === "staff" && user.permissions?.[permission] === false)
    return (
      <Navigate
        to={
          user.permissions?.dashboard !== false
            ? "/"
            : user.permissions?.interviews !== false
              ? "/interviews"
              : "/no-access"
        }
        replace
      ...
      <Suspense fallback={<div className="center"><span className="loader" /></div>}><Routes>
        ...
        >
          ...
          <Route
            path="interviews"
            element={
              <PermissionGuard permission="interviews">
                <Interviews />
              </PermissionGuard>
            }
          />
          <Route
            path="interviews/new"
            element={
              <Guard roles={["user", "staff"]}>
                <PermissionGuard permission="createInterview">
                  <InterviewForm />
                </PermissionGuard>
              </Guard>
            }
          ...
          <Route
            path="google-sheet"
            element={
              <PermissionGuard permission="googleSheet">
                <GoogleSheet />
              </PermissionGuard>
            }
          />
          ...
          <Route
            ...
            element={
              <div className="panel empty">
                No workspace sections are enabled for this staff account.
