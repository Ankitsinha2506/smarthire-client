...
import {ClipboardList,CalendarCheck2,Trophy,UsersRound,Sparkles,ArrowUpRight} from 'lucide-react';
import {api} from '../api';
...
export default function StaffAnalytics({user}){
  const greeting=dashboardGreeting(user),[period,setPeriod]=useState('all'),[stats,setStats]=useState(null),[work,setWork]=useState({total:0,interviews:[]}),[todayWork,setTodayWork]=useState([]),[todaySchedule,setTodaySchedule]=useState([]),[tomorrowWork,setTomorrowWork]=useState([]),[loading,setLoading]=useState(false);
  useEffect(()=>{
    ...
    Promise.all([
      ...
      api('/interviews/staff-workload?'+tomorrowParams),sheetEnabled?api('/google-sheet/normalized?'+tomorrowParams):Promise.resolve({items:[]})
    ]).then(([summary,periodDatabase,periodSheet,todayDatabase,allTodayDatabase,todaySheet,tomorrowDatabase,tomorrowSheet])=>{
      ...
      setTomorrowWork(mergeInterviews(tomorrowDatabase.interviews,tomorrowSheet.items,user.id));
  ...
  return <div className={loading?'staff-analytics bi-loading':'staff-analytics'}>
    ...
    <TomorrowSchedule items={tomorrowWork} showSupport/>
